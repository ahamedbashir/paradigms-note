...
; sometimes accumulate is referred to as fold-right

(define (fold-right op init seq)
  (cond ((null? seq) init)
        (else (op (car seq) (fold-right op init (cdr seq))))))

; as you might expect from this, there is also a function fold-left
...
(define (quicksort lst)
  (cond ((null? lst) lst)
        (else
         (let* ((x (car lst))
                (smallerSorted (quicksort (filter (lambda (y) (< y x)) (cdr lst))))
                (largerSorted (quicksort (filter (lambda (y) (> y x)) (cdr lst)))))
           (append
            smallerSorted
            (list x)
            largerSorted)))))
...
; what does this next function count?
...
(define (count-nodes-3 t)
  (cond ((null? t) 0)
        ((atom? t) 1)
        ((atom? (car t)) (+ 1 (count-nodes-3 (cdr t))))
        (else (+ (count-nodes-3 (car t)) (count-nodes-3 (cdr t))))))
...
(define (count-nodes-4 tree)
  (+ 1 (accumulate + 0 (map (lambda (subtree)
			      (if (pair? subtree)
				  (count-nodes-4 subtree)
				  1))
			    tree))))

; refining further -- what about this?

(define (count-nodes-5 tree)
  (if (atom? tree)
      1
      (+ 1 (accumulate + 0 (map (lambda (subtree) (count-nodes-5 subtree)) tree)))))
